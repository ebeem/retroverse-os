...
DEBUGPREFIX = '[ chainsgenocide {0} | {1} | {2} | DEBUG ]'.format(version, kodi_version, sys_platform)
INFOPREFIX = '[ chainsgenocide | INFO ]'
LOGPATH = control.transPath('special://logpath/')
log_file = os.path.join(LOGPATH, 'chainsgenocide.log')
...
def log(msg, trace=0):
    ...
    try:
        ...
        try:
            xbmc.log('chainsgenocide Logging Failure: %s' % e, LOGDEBUG)
        except:
            pass
...
def upload_log():
    ...
    if not data:
        ...
        UserAgent = 'chainsgenocide %s' % version
        try:
