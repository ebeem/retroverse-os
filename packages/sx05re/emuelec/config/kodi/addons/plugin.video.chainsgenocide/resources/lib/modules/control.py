...
def artPath():
    theme = appearance()
    if theme in ['-', '']: return
    elif condVisibility('System.HasAddon(script.chainsgenocide.artwork)'):
        return os.path.join(xbmcaddon.Addon('script.chainsgenocide.artwork').getAddonInfo('path'), 'resources', 'media', theme)


def appearance():
    appearance = setting('appearance.1').lower() if condVisibility('System.HasAddon(script.chainsgenocide.artwork)') else setting('appearance.alt').lower()
    return appearance


def artwork():
    execute('RunPlugin(plugin://script.chainsgenocide.artwork)')
...
# def metaFile():
    # if condVisibility('System.HasAddon(script.chainsgenocide.metadata)'):
        # return os.path.join(xbmcaddon.Addon('script.chainsgenocide.metadata').getAddonInfo('path'), 'resources', 'data', 'meta.db')
...
def clean_settings(info=True):#Fen code
    ...
    def _make_content(dict_object):
        ...
        return content
    try:
        for addon_id in ('plugin.video.chainsgenocide', 'script.module.chainsgenocidescrapers'):
            removed_settings = []
            active_settings = []
