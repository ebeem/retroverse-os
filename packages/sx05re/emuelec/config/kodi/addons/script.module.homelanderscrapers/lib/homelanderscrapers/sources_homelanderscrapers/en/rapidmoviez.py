...
from homelanderscrapers.modules import dom_parser
    ...
    def _get_sources(self, name, url):
        try:
            ...
            for url in urls:
                ...
                if not valid:
                    continue
                self.sources.append({'source': host, 'quality': quality, 'language': 'en', 'url': url, 'info': info, 'direct': False, 'debridonly': False, 'size': dsize, 'name': name})
        except:
            #log_utils.log('RMZ - Exception', 1)
            pass
